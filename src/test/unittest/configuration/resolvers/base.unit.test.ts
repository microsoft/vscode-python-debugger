...
import { AttachRequestArguments, DebugOptions, LaunchRequestArguments } from '../../../../extension/types';
...
suite('Debugging - Config Resolver', () => {
    class BaseResolver extends BaseConfigurationResolver<AttachRequestArguments | LaunchRequestArguments> {
        ...
        public getPythonPathSource() {
            return this.pythonPathSource;
        }
    }
...
suite('resolveAndUpdatePythonPath prioritization tests', () => {
    class BaseResolver2 extends BaseConfigurationResolver<AttachRequestArguments | LaunchRequestArguments> {
        public resolveDebugConfiguration(
            _folder: WorkspaceFolder | undefined,
            _debugConfiguration: DebugConfiguration,
            _token?: CancellationToken,
    ...
    let resolver: BaseResolver2;
    let getInterpreterDetailsStub: sinon.SinonStub;

    setup(() => {
        resolver = new BaseResolver2();
        getInterpreterDetailsStub = sinon.stub(pythonApi, 'getInterpreterDetails');
    });
    ...
    test('debugAdapterPython should use pythonPath when pythonPath is set but python has different value', async () => {
        const pythonPathValue = path.join('path', 'to', 'pythonPath', 'python');
        const pythonValue = path.join('path', 'to', 'python', 'python');
        const config = {
            pythonPath: pythonPathValue,
    ...
    test('debugLauncherPython should use pythonPath when pythonPath is set but python has different value', async () => {
        const pythonPathValue = path.join('path', 'to', 'pythonPath', 'python');
        const pythonValue = path.join('path', 'to', 'python', 'python');
        const config = {
            pythonPath: pythonPathValue,
    ...
    test('When pythonPath is undefined and python is concrete path, debugAdapter and debugLauncher should use resolved pythonPath from interpreter', async () => {
        const pythonValue = path.join('path', 'to', 'python', 'python');
        const interpreterPath = path.join('path', 'from', 'interpreter');
        const config = {
            python: pythonValue,
