...
import { PythonEnvironment } from '../debugger/adapter/types';
...
interface IExtensionApi {
    ...
    environment: {
        getEnvironmentDetails(path: string | EnvironmentPath): PythonEnvironment;
    }
}
...
export async function getEnvironmentDetails(path: string | EnvironmentPath) {
    const api = await getPythonExtensionAPI();
    return api?.environment.getEnvironmentDetails(path);
}
